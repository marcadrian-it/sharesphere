...
export class Initial1682784186107 implements MigrationInterface {
  name = "Initial1682784186107";

  public async up(queryRunner: QueryRunner): Promise<void> {
    const tableExistsUpvote = await queryRunner.hasTable("upvote");
    if (!tableExistsUpvote) {
      await queryRunner.query(
        `CREATE TABLE "upvote" ("value" integer NOT NULL, "userId" integer NOT NULL, "postId" integer NOT NULL, CONSTRAINT "PK_802ac6b9099f86aa24eb22d9c05" PRIMARY KEY ("userId", "postId"))`
      );
      await queryRunner.query(
    ...
    const tableExistsUser = await queryRunner.hasTable("user");
    if (!tableExistsUser) {
      await queryRunner.query(
    ...
    const tableExistsPost = await queryRunner.hasTable("post");
    if (!tableExistsPost) {
      await queryRunner.query(
        `CREATE TABLE "post" ("id" SERIAL NOT NULL, "title" character varying NOT NULL, "text" character varying NOT NULL, "points" integer NOT NULL DEFAULT '0', "authorId" integer NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_be5fda3aac270b134ff9c21cdee" PRIMARY KEY ("id"))`
      );
      await queryRunner.query(
